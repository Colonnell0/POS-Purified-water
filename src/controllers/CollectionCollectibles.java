 ...
 * @author Colonnello
 */
public class CollectionCollectibles extends Customer implements Initializable {
    ...
    private String transactionType = "Collection & Collectibles";
    ...
    public void tabulateData() {
        ...
        if (tgbTotal.isSelected()) {
            tblCustomer.getItems().clear();
            String res[][] = viewCollectionCollectiblesGroup(login_name, customerName, from, to, barangay, city, province, transactionType);
            if (res != null) {
                int i = 1;
                for (String[] re : res) {
        ...
        if (!tgbTotal.isSelected()) {
            tblCustomer.getItems().clear();
            String res[][] = viewCollectionCollectibles(login_name, customerName, from, to, barangay, city, province, transactionType);
            if (res != null) {
                int i = 1;
                for (String[] re : res) {
                    Customer_Transaction_Model entry = new Customer_Transaction_Model(
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            login_name = setLoginDetails()[1];
            System.out.println(login_name);
        } catch (UnknownHostException ex) {
            Logger.getLogger(CollectionCollectibles.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        cmbTransaction.setItems(obList);

    }

    @FXML
    public void cmbTransactionHandler() {
        transactionType = cmbTransaction.getValue().toString();
        tabulateData();
    }

    @FXML
    public void cmbColumnsHandler() {
        print_start = Integer.parseInt(cmbNumberOfColumns.getValue().toString().split("\\s+")[0]);
        print_end = Integer.parseInt(cmbNumberOfColumns.getValue().toString().split("\\s+")[2]);
    ...
    public void print() throws IOException {
        ...
        loader.setLocation(Main.class.getResource("/views/PrintPreviewCollectionCollectibles.fxml"));
        ...
        PrintPreviewCollectionCollectiblesController controller = loader.<PrintPreviewCollectionCollectiblesController>getController();
        ...
        for (int i = print_start - 1; i < print_end; i++) {
            ...
            if (transactionType.equalsIgnoreCase("Collection") || transactionType.equalsIgnoreCase("Collection & Collectibles")) {
                printAmount = tblCustomer.getItems().get(i).getCustomerAmount();
            }
            if (transactionType.equalsIgnoreCase("Collectibles")) {
                printAmount = tblCustomer.getItems().get(i).getCustomerBalance();
            }
